...
create table public.village_merge_map (
  ...
  new_village_id uuid references public.villages(id) on update cascade on delete restrict,
  proposed_new_village_id uuid references public.villages(id) on update cascade on delete restrict,
  mapping_status text not null default 'confirmed',
  mapping_version text not null default 'unversioned',
  source_note text,
  ...
  constraint village_merge_map_target_check check (
...
create table public.villages_legacy (
  ...
  proposed_dissolved_into_village_id uuid references public.villages(id) on update cascade on delete set null,
  dissolved_date date,
  commune_id text not null,
  mapping_status text not null default 'confirmed',
  mapping_version text not null default 'unversioned',
  legacy_unit_type text not null default 'village',
  note text,
  constraint villages_legacy_name_not_blank check (btrim(old_name) <> ''),
  constraint villages_legacy_status_check check (
    mapping_status in ('confirmed', 'pending_official_decision')
  ),
  constraint villages_legacy_unit_type_check check (
    legacy_unit_type in ('village', 'resettlement_area')
  ),
  constraint villages_legacy_target_check check (
...
create table public.report_periods (
  ...
  constraint report_periods_template_sha256_check check (
    template_sha256 is null or template_sha256 ~ '^[0-9a-f]{64}$'
  ),
  constraint report_periods_template_size_check check (
    template_size_bytes is null or template_size_bytes between 1 and 5242880
  ),
  constraint report_periods_template_metadata_check check (
...
create table public.report_period_change_requests (
  ...
  constraint report_period_change_requests_commune_not_blank check (btrim(commune_id) <> ''),
  constraint report_period_change_requests_kind_check check (request_kind in ('update', 'delete')),
  constraint report_period_change_requests_reason_check check (char_length(btrim(reason)) between 10 and 1000),
  constraint report_period_change_requests_before_object check (jsonb_typeof(before_snapshot) = 'object'),
...
create table public.report_period_change_decisions (
  id uuid primary key default gen_random_uuid(),
  request_id uuid not null unique references public.report_period_change_requests(id) on delete restrict,
  decision text not null,
  reason text not null,
  decided_by uuid not null references public.user_profiles(id) on delete restrict,
  decided_at timestamptz not null default now(),
  constraint report_period_change_decisions_value_check check (decision in ('approved', 'rejected')),
  constraint report_period_change_decisions_reason_check check (char_length(btrim(reason)) between 5 and 1000)
);
...
create function public.reject_immutable_report_period_history()
returns trigger
language plpgsql
security definer
set search_path = pg_catalog, public
...
create function public.reject_archived_period_report_mutation()
returns trigger
language plpgsql
security definer
set search_path = pg_catalog, public
...
create function public.report_indicator_values_are_valid(target_report_id uuid)
returns boolean
language sql
stable
set search_path = pg_catalog, public
...
grant select on public.user_profiles, public.user_village_assignments,
  public.villages_legacy, public.report_periods, public.report_period_villages,
  public.report_period_change_requests, public.report_period_change_decisions
  to authenticated;
...
create function public.attach_report_period_template(
  p_period_id uuid,
...
begin
  if actor is null or public.profile_role() <> 'admin_xa'
  ...
  set template_name = btrim(p_template_name), template_path = btrim(p_template_path),
      template_sha256 = p_template_sha256, template_size_bytes = p_template_size_bytes
  ...
  insert into public.audit_log (commune_id, action, table_name, record_id, user_id, details)
  values (target.commune_id, 'ATTACH_REPORT_PERIOD_TEMPLATE', 'report_periods',
    target.id, actor, jsonb_build_object(
...
begin
  ...
  select period.* into target from public.report_periods period
  where period.id = p_period_id and period.commune_id = actor_commune
  for update;
  if target.id is null then
  ...
  if exists (
    select 1 from public.report_period_change_requests request
    left join public.report_period_change_decisions decision on decision.request_id = request.id
    where request.period_id = target.id and decision.id is null
  ) then
    raise exception 'period_change_request_pending' using errcode = '23505';
  end if;

  select coalesce(jsonb_agg(assignment.village_id::text order by assignment.village_id::text), '[]'::jsonb)
  into current_villages from public.report_period_villages assignment
  where assignment.period_id = target.id;
  before_state := jsonb_build_object(
    'name', target.name, 'due_date', target.due_date, 'village_ids', current_villages);

  if p_request_kind = 'delete' then
    if p_proposed_name is not null or p_proposed_due_date is not null
    ...
    if p_proposed_village_ids is not null then
      ...
      select count(*) into valid_count from public.villages village
      where village.id = any(p_proposed_village_ids)
        and village.commune_id = actor_commune and village.is_active;
      if requested_count <> cardinality(p_proposed_village_ids)
         or valid_count <> requested_count then
        raise exception 'villages_must_be_unique_active_and_in_commune' using errcode = '22023';
      end if;
      if exists (
        select 1 from public.reports report where report.period_id = target.id
      ...
      into proposed_villages from unnest(p_proposed_village_ids) village_id;
      final_villages := proposed_villages;
    end if;
    proposed_state := jsonb_build_object(
      'name', final_name, 'due_date', final_due_date, 'village_ids', final_villages);
  ...
  insert into public.report_period_change_requests (
    commune_id, period_id, request_kind, reason, before_snapshot, proposed_snapshot, requested_by
  ) values (
    actor_commune, target.id, p_request_kind, btrim(p_reason), before_state, proposed_state, actor
  ) returning * into created_request;
  insert into public.audit_log (commune_id, action, table_name, record_id, user_id, details)
  values (actor_commune,
    case when p_request_kind = 'delete' then 'REQUEST_REPORT_PERIOD_DELETE'
      else 'REQUEST_REPORT_PERIOD_UPDATE' end,
    'report_period_change_requests', created_request.id, actor,
    jsonb_build_object('period_id', target.id, 'reason', created_request.reason,
      'before', before_state, 'proposed', proposed_state));
  insert into public.notifications (user_id, title, body, url)
  select leader.id, 'Yêu cầu thay đổi kỳ báo cáo cần phê duyệt',
    format('%s đề nghị %s kỳ %s',
      ...
      target.name), '/app/period-change-requests'
  from public.user_profiles leader
  where leader.commune_id = actor_commune and leader.role = 'lanh_dao'
    and leader.is_active and not leader.force_password_reset;
...
begin
  if actor is null or public.profile_role() <> 'lanh_dao'
     or not coalesce(public.profile_can_mutate(), false) then
    raise exception 'forbidden' using errcode = '42501';
  end if;
  ...
  select request.* into target_request from public.report_period_change_requests request
  where request.id = p_request_id and request.commune_id = actor_commune for update;
  if target_request.id is null then
    raise exception 'request_not_found' using errcode = 'P0002';
  end if;
  if exists (select 1 from public.report_period_change_decisions decision
    where decision.request_id = target_request.id) then
    raise exception 'request_already_decided' using errcode = '23505';
  end if;
  select period.* into target_period from public.report_periods period
  where period.id = target_request.period_id and period.commune_id = actor_commune for update;
  if target_period.id is null then
    raise exception 'period_not_found' using errcode = 'P0002';
  end if;
  insert into public.report_period_change_decisions (request_id, decision, reason, decided_by)
  values (target_request.id, p_decision, btrim(p_reason), actor)
  ...
  if p_decision = 'approved' and target_request.request_kind = 'update' then
    ...
    select array_agg(value::uuid order by value::uuid) into proposed_village_ids
    from jsonb_array_elements_text(target_request.proposed_snapshot -> 'village_ids') value;
    update public.report_periods set name = proposed_name, due_date = proposed_due_date
    where id = target_period.id and archived_at is null;
    if not found then raise exception 'period_already_archived' using errcode = '23505'; end if;
    delete from public.report_period_villages where period_id = target_period.id
      and not (village_id = any(proposed_village_ids));
    insert into public.report_period_villages (period_id, village_id)
    select target_period.id, village_id from unnest(proposed_village_ids) village_id
    on conflict do nothing;
  elsif p_decision = 'approved' and target_request.request_kind = 'delete' then
    update public.report_periods set archived_at = created_decision.decided_at,
  ...
  values (actor_commune,
    ...
    jsonb_build_object('request_id', target_request.id, 'period_id', target_request.period_id,
      'reason', created_decision.reason, 'before', target_request.before_snapshot,
      'proposed', target_request.proposed_snapshot));
  insert into public.notifications (user_id, title, body, url)
  values (target_request.requested_by,
    case when p_decision = 'approved' then 'Yêu cầu thay đổi kỳ báo cáo đã được phê duyệt'
    ...
    format('Lãnh đạo đã %s yêu cầu %s. Lý do: %s',
      ...
      case when target_request.request_kind = 'delete' then 'lưu trữ kỳ báo cáo'
        else 'điều chỉnh kỳ báo cáo' end, created_decision.reason),
    '/app/create-period');
...
create table public.ai_action_drafts (
  ...
  constraint ai_drafts_review_metadata check (
    (
...
revoke all on function public.enforce_ai_action_draft_integrity() from public, anon, authenticated, service_role;
...
create policy ai_drafts_insert_admin on public.ai_action_drafts for insert to authenticated with check (
  public.profile_role() = 'admin_xa'
  and public.profile_can_mutate()
  and commune_id = public.profile_commune_id()
...
create table public.report_import_batches (
  ...
  expected_village_count integer not null default 22 check (expected_village_count > 0),
  created_by uuid not null references public.user_profiles(id) on delete restrict,
  committed_by uuid references public.user_profiles(id) on delete restrict,
  ...
  constraint report_import_batches_commit_metadata check (
    (status = 'committed' and committed_by is not null and committed_at is not null)
    or status <> 'committed'
  )
);

create table public.report_import_files (
  id uuid primary key default gen_random_uuid(),
  batch_id uuid not null references public.report_import_batches(id) on delete cascade,
  source_filename text not null,
  content_sha256 text not null,
  size_bytes integer not null check (size_bytes > 0),
  source_village_name text not null,
  legacy_village_id uuid references public.villages_legacy(id) on delete restrict,
  ...
  report_source public.report_source not null default 'excel',
  ...
  constraint report_import_files_json_shapes check (
    jsonb_typeof(metadata) = 'object'
    and jsonb_typeof(raw_values) = 'object'
    and jsonb_typeof(normalized_values) = 'object'
    and jsonb_typeof(validation_flags) = 'array'
  ),
  constraint report_import_files_target_check check (
    (mapping_status = 'confirmed' and target_village_id is not null)
    or (mapping_status in ('pending_official_decision', 'unmapped') and target_village_id is null)
  ),
...
create table public.report_import_resolutions (
  id uuid primary key default gen_random_uuid(),
  import_file_id uuid not null references public.report_import_files(id) on delete cascade,
  ct_code text not null check (ct_code ~ '^CT(0[1-9]|1[0-4])$'),
...
create table public.report_import_lineage (
  report_id uuid not null references public.reports(id) on delete cascade,
  import_file_id uuid not null references public.report_import_files(id) on delete restrict,
  created_at timestamptz not null default now(),
  primary key (report_id, import_file_id)
);

create function public.commit_report_import_batch(p_batch_id uuid)
...
begin
  ...
  where target.commune_id = target_batch.commune_id
    and exists (
      ...
      where legacy.commune_id = target_batch.commune_id and legacy.mapping_version = target_batch.mapping_version
        and legacy.legacy_unit_type = 'village' and legacy.mapping_status = 'confirmed'
  ...
  if exists (
    ...
    where report.period_id = target_batch.period_id and report.workflow_status not in ('draft', 'needs_revision')
...
create policy report_import_batches_select_internal on public.report_import_batches for select to authenticated using (
  public.profile_role() in ('admin_xa', 'lanh_dao') and commune_id = public.profile_commune_id()
);
create policy report_import_batches_mutate_admin on public.report_import_batches for all to authenticated using (
  public.profile_role() = 'admin_xa' and public.profile_can_mutate() and commune_id = public.profile_commune_id()
) with check (
  public.profile_role() = 'admin_xa' and public.profile_can_mutate() and commune_id = public.profile_commune_id()
);
...
create policy report_import_files_mutate_admin on public.report_import_files for all to authenticated using (
  public.profile_role() = 'admin_xa' and public.profile_can_mutate()
  and exists (select 1 from public.report_import_batches batch where batch.id = batch_id and batch.commune_id = public.profile_commune_id())
) with check (
  public.profile_role() = 'admin_xa' and public.profile_can_mutate()
  and exists (select 1 from public.report_import_batches batch where batch.id = batch_id and batch.commune_id = public.profile_commune_id())
);
...
create policy report_import_lineage_select_internal on public.report_import_lineage for select to authenticated using (
  public.can_select_report(report_id)
);
...
-- Supabase Storage is optional in plain PostgreSQL test environments. In a
-- Supabase project the bucket is private, immutable-by-hash, and caller scoped.
do $storage$
begin
...
create index field_synonyms_commune_code_idx
