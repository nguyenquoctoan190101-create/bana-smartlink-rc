  ...
  return <div className="knowledge-page knowledge-hub space-y-6">
    ...
    <div className="knowledge-two-column">
      <SectionCard className="knowledge-section knowledge-section--champions"><SectionTitle tone="people" icon={<Users />} title="Đại sứ số" description="Cán bộ hỗ trợ người dân và nhóm yếu thế tiếp cận dịch vụ số." count={`${champions.length} người`} />
        <div className="knowledge-section__body">{champions.length ? <ItemList>{champions.map((champion) => <li key={champion.id}><strong>{officers.find((officer) => officer.id === champion.user_id)?.name || "Cán bộ được phân công"}</strong><div className="knowledge-detail-grid"><span><b>Kỹ năng</b>{champion.skills?.join(", ") || "Chưa cập nhật"}</span><span><b>Lịch hỗ trợ</b>{champion.support_schedule || "Chưa cập nhật"}</span><span><b>Nhóm hỗ trợ</b>{champion.supported_groups || "Chưa cập nhật"}</span></div></li>)}</ItemList> : <EmptyState title="Chưa có Đại sứ số" description="Admin chọn cán bộ trong danh sách để phân công hỗ trợ." />}
      ...
      <SectionCard className="knowledge-section knowledge-section--points"><SectionTitle tone="location" icon={<MapPin />} title="Điểm hỗ trợ cộng đồng" description="Địa điểm, lịch trực và thiết bị dùng chung phục vụ người dân." count={`${points.length} điểm`} />
        <div className="knowledge-section__body">{points.length ? <ItemList>{points.map((point) => <li key={point.id}><strong>{point.name}</strong><div className="knowledge-detail-grid"><span><b>Địa chỉ</b>{point.address}</span><span><b>Lịch trực</b>{point.opening_hours || "Chưa cập nhật"}</span><span><b>Thiết bị</b>{point.equipment?.join(", ") || "Chưa cập nhật"}</span></div></li>)}</ItemList> : <EmptyState title="Chưa có điểm hỗ trợ" description="Admin cấu hình địa điểm và thiết bị dùng chung trước khi công khai nội bộ." />}
