...
type EvacuationPoint = { id: string; village_id: string; name: string; latitude: number; longitude: number; capacity_households: number; contact_name: string; contact_phone: string | null; is_verified: boolean };
...
export default function KnowledgeCenter({ role, scenarioEnabled = false }: Props) {
  ...
  const createEvacuationPoint = () => submit(async () => {
    ...
    await apiJson<EvacuationPoint>("/api/pilots/evacuation-points", { method: "POST", body: JSON.stringify({ village_id: evacuationVillageId, name: evacuationName.trim(), latitude, longitude, capacity_households: capacity, contact_name: evacuationContact.trim() || "Trực ban UBND xã", contact_phone: evacuationPhone.trim() || null }) });
  ...
  return <div className="knowledge-page knowledge-hub space-y-6">
    ...
    <div className="knowledge-two-column">
      ...
      <SectionCard className="knowledge-section knowledge-section--points"><SectionTitle tone="location" icon={<MapPin />} title="Điểm hỗ trợ cộng đồng" description="Địa điểm, lịch trực và thiết bị dùng chung phục vụ người dân." count={`${points.length} điểm`} />
        <div className="knowledge-section__body">{points.length ? <ItemList>{points.map((point) => <li key={point.id}><div className="knowledge-item__title">{point.name}</div><div className="knowledge-detail-grid"><div><span className="knowledge-detail-grid__label">Địa chỉ</span><span>{point.address}</span></div><div><span className="knowledge-detail-grid__label">Lịch trực</span><span>{point.opening_hours || "Chưa cập nhật"}</span></div><div><span className="knowledge-detail-grid__label">Thiết bị</span><span>{point.equipment?.join(", ") || "Chưa cập nhật"}</span></div><div><span className="knowledge-detail-grid__label">Đại sứ số phụ trách</span><span>{point.champion_id ? "Đã phân công" : "Chưa phân công"}</span></div></div></li>)}</ItemList> : <EmptyState title="Chưa có điểm hỗ trợ" description="Quản trị xã cấu hình địa điểm và thiết bị dùng chung trước khi công khai nội bộ." />}
