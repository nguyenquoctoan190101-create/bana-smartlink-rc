    ...
    <SectionCard className="overflow-hidden">
      <div className="border-b border-slate-200 p-5 md:p-7">
        <ol className="grid grid-cols-3 gap-2" aria-label="Tiến trình gửi đề nghị đối chiếu số liệu">
          {["Số liệu", "Liên hệ", "Xác nhận"].map((label, index) => { const number = index + 1; const active = step >= number; return <li key={label} aria-current={step === number ? "step" : undefined} className={`flex min-w-0 flex-col items-start gap-2 border-t-2 pt-3 text-xs font-semibold sm:flex-row sm:items-center ${active ? "border-emerald-700 text-emerald-900" : "border-slate-200 text-slate-400"}`}><span className={`grid h-6 w-6 place-items-center rounded-full ${active ? "bg-emerald-800 text-white" : "bg-slate-100"}`}>{step > number ? <Check className="h-3.5 w-3.5" /> : number}</span>{label}</li>; })}
        </ol>
      </div>
      <form onSubmit={submit} className="p-5 md:p-7">
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><Button type="button" variant="secondary" className="w-full justify-center sm:w-auto" onClick={() => moveTo(1)}><ArrowLeft />Quay lại</Button><Button type="button" className="w-full justify-center sm:w-auto" onClick={() => moveTo(3)}>Tiếp tục<ArrowRight /></Button></div>
          </div>
        )}
        {step === 3 && <div className="space-y-5"><div className="rounded-lg border border-slate-200 bg-slate-50 p-4"><h3 className="font-bold text-slate-900">Xác nhận nội dung</h3><dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-slate-500">Phạm vi</dt><dd className="font-semibold text-slate-900">{villageName} · {reportName}</dd></div><div><dt className="text-slate-500">Chỉ tiêu</dt><dd className="font-semibold text-slate-900">{selectedIndicator} · {indicatorNames[selectedIndicator]}</dd></div><div><dt className="text-slate-500">Giá trị đang công bố</dt><dd className="font-semibold text-slate-900">{hasPublishedValue ? `${Number(publishedValue).toLocaleString("vi-VN")} ${indicatorUnits[selectedIndicator]}` : "Chưa có dữ liệu"}</dd></div><div><dt className="text-slate-500">Giá trị đề xuất</dt><dd className="font-semibold text-slate-900">{Number(suggestedValue).toLocaleString("vi-VN")} {indicatorUnits[selectedIndicator]}</dd></div><div className="sm:col-span-2"><dt className="text-slate-500">Lý do</dt><dd className="font-semibold text-slate-900">{explanation}</dd></div></dl></div><label className="flex min-h-14 items-start gap-3 rounded-lg border border-slate-200 p-4 text-sm text-slate-700"><input className="mt-0.5 h-5! min-h-5! w-5!" type="checkbox" checked={privacyConsent} onChange={(event) => setPrivacyConsent(event.target.checked)} required /><span>Tôi đồng ý gửi số điện thoại, thông tin tùy chọn đã nhập và nội dung đề nghị để UBND xã Bà Nà xử lý theo thông báo quyền riêng tư.</span></label><div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><Button type="button" variant="secondary" className="w-full justify-center sm:w-auto" onClick={() => moveTo(2)}><ArrowLeft />Quay lại</Button><Button type="submit" className="w-full justify-center sm:w-auto" disabled={isSending || !privacyConsent}>{isSending ? <RefreshCw className="animate-spin" /> : <ClipboardCheck />}Gửi đề nghị đối chiếu</Button></div></div>}
        {step === 4 && <div className="py-6 text-center"><span className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-emerald-100 text-emerald-800"><CheckCircle2 className="h-8 w-8" /></span><h3 className="mt-5 text-xl font-bold text-slate-900">Đề nghị đối chiếu đã được ghi nhận</h3><p className="mx-auto mt-2 max-w-lg text-sm text-slate-600">Lưu mã bên dưới để theo dõi trạng thái. Thông tin cá nhân không được lặp lại trên màn hình xác nhận.</p>{trackingCode && <div className="mx-auto mt-5 max-w-sm rounded-lg border border-emerald-200 bg-emerald-50 p-4"><p className="text-xs font-bold text-emerald-800">MÃ TRA CỨU</p><p className="mt-1 break-all select-all font-mono text-xl font-bold tracking-wider text-emerald-950">{trackingCode}</p><Button type="button" variant="secondary" className="mt-3 w-full justify-center sm:w-auto" onClick={() => void copyTrackingCode()}><Copy />{trackingCodeCopied ? "Đã sao chép" : "Sao chép mã"}</Button></div>}<Button type="button" className="mt-6 w-full justify-center sm:w-auto" onClick={reset}><MessageSquare />Gửi đề nghị khác</Button></div>}
      </form>
    </SectionCard>
  </div>;
}
