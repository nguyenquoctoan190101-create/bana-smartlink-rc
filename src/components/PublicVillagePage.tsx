...
import { formatPublicLookupMessage, getPublicCaseCategoryLabel, getPublicLookupEndpoint, getPublicStatusLabel } from "../lib/publicLookup";
...
export function getDefaultPublicVillageId(villages: unknown[], reports: unknown[]): string {
  const publishedVillageIds = new Set(reports.map((report) => (report && typeof report === "object" ? (report as { village_id?: unknown }).village_id : null)).filter((id): id is string => typeof id === "string" && id.length > 0));
...
export default function PublicVillagePage({ onGoToLogin }: PublicVillagePageProps) {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      setDataError(null);
      try {
        const [villageData, reportData, evacuationData] = await Promise.all([loadVillages(), apiJson<unknown[]>("/reports/public"), apiJson<unknown[]>("/api/pilots/evacuation-points").catch(() => [])]);
        if (!active) return;
    ...
    <div className="mx-auto max-w-[1440px] space-y-6" id="public-village-portal">
      <section className={`public-hero relative overflow-hidden rounded-xl bg-[#0f5a48] px-5 py-8 text-white md:px-10 md:py-12 ${mode === "data" ? "" : "public-hero--compact"}`}>
        <picture className="public-hero-photo" aria-hidden="true">
          <source type="image/webp" srcSet="/images/ba-na/ba-na-hero-golden-bridge-960.webp 960w, /images/ba-na/ba-na-hero-golden-bridge-1920.webp 1920w" sizes="(max-width: 767px) 100vw, (max-width: 1536px) 94vw, 1480px" />
          <img src="/images/ba-na/ba-na-hero-golden-bridge.jpg" width="1920" height="1078" alt="" decoding="async" fetchPriority="high" />
        </picture>
        ...
        <div className="public-hero-nav relative z-10 mt-7 flex flex-wrap gap-3" aria-label="Điều hướng cổng công khai">
          <Button aria-pressed={mode === "data"} variant={mode === "data" ? "secondary" : "quiet"} className={mode !== "data" ? "border border-white/35 bg-white/10 text-white hover:bg-white/20 hover:text-white" : ""} onClick={() => setMode("data")}>
            <FileText />
            Số liệu công khai
          </Button>
          <Button
            ...
            className={mode !== "proposal" ? "border border-white/35 bg-white/10 text-white hover:bg-white/20 hover:text-white" : ""}
            onClick={() => {
              setMode("proposal");
              setProposalStep(1);
            }}
          ...
          <Button aria-pressed={mode === "case"} variant={mode === "case" ? "secondary" : "quiet"} className={mode !== "case" ? "border border-white/35 bg-white/10 text-white hover:bg-white/20 hover:text-white" : ""} onClick={() => setMode("case")}>
            <MapPin />
            Phản ánh hiện trường
          </Button>
          <Button aria-pressed={mode === "lookup"} variant={mode === "lookup" ? "secondary" : "quiet"} className={mode !== "lookup" ? "border border-white/35 bg-white/10 text-white hover:bg-white/20 hover:text-white" : ""} onClick={() => setMode("lookup")}>
            <FileSearch />
            Tra cứu hồ sơ
          </Button>
          {onGoToLogin && (
            <Button variant="quiet" className="border border-white/35 bg-white/10 text-white hover:bg-white/20 hover:text-white" onClick={onGoToLogin}>
              <Lock />
              Đăng nhập cán bộ
            </Button>
          )}
      ...
      {mode === "data" && (
        <div className="space-y-5">
          <FilterBar>
            <div className="grid flex-1 gap-3 sm:grid-cols-2">
              <label className="text-sm font-semibold text-slate-700">
                ...
                <select value={selectedVillageId} onChange={(event) => setSelectedVillageId(event.target.value)} disabled={!villages.length}>
                  {!villages.length && <option value="">Chưa có danh mục thôn</option>}
                  {villages.map((item) => (
          ...
          {isLoading ? (
            <div role="status" className="flex min-h-48 items-center justify-center gap-2 text-slate-600">
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
              {PUBLIC_INDICATORS.map((indicator) => {
                ...
                return <MetricCard key={indicator.code} label={`${indicator.code} · ${indicator.name}`} value={formatPublicIndicatorValue(rawValue)} unit={hasValue ? indicator.unit : undefined} context={hasValue ? `Nguồn: bản công bố ${periodLabel}` : "Chưa có dữ liệu được công bố"} tone={indicator.tone} icon={<Icon />} />;
              })}
            </div>
          )}
          ...
          <SectionCard className="flex flex-col gap-4 p-5 md:flex-row md:items-start">
            ...
            <div>
              <h2 className="font-bold text-slate-900">Phạm vi công khai và quyền riêng tư</h2>
              <p className="mt-1 text-sm leading-relaxed text-slate-600">Cổng chỉ hiển thị CT01, CT02, CT09, CT12 và CT13 sau khi được công bố. CT14 và dữ liệu nhận diện cá nhân không xuất hiện trong phản hồi công khai.</p>
            </div>
          </SectionCard>
          ...
          <SectionCard className="p-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="font-bold text-slate-900">Điểm sơ tán đã công bố</h2>
                <p className="mt-1 text-sm leading-relaxed text-slate-600">Danh mục chuẩn bị ứng phó do cơ quan có thẩm quyền phê duyệt và công bố. Đây không phải kênh phát cảnh báo khẩn cấp; khi có sự cố, hãy theo hướng dẫn chính thức.</p>
              </div>
            </div>
            {evacuationPoints.length === 0 ? (
              <div className="mt-4">
                <EmptyState title="Chưa có điểm sơ tán công khai" description="Danh mục sẽ hiển thị sau khi được cơ quan có thẩm quyền duyệt và công bố." />
              </div>
            ) : (
              <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {evacuationPoints.map((point) => {
                  const pointVillage = villages.find((village) => village.id === point.village_id)?.name || "Toàn xã";
                  return (
                    <article key={point.id} className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-4">
                      <div className="flex items-start justify-between gap-2">
                        <h3 className="font-semibold text-slate-900">{point.name}</h3>
                        <StatusBadge status="approved" label="Đã công bố" />
                      </div>
                      <p className="mt-2 text-sm text-slate-700">{pointVillage}</p>
                      <p className="mt-1 text-sm text-slate-600">
                        Sức chứa dự kiến: <strong>{point.capacity_households.toLocaleString("vi-VN")} hộ</strong>
                      </p>
                      <a className="mt-3 inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-emerald-800 underline-offset-4 hover:underline" href={`https://www.google.com/maps/search/?api=1&query=${point.latitude},${point.longitude}`} target="_blank" rel="noreferrer">
            ...
            <div className="public-place-story__grid">
              <figure className="public-place-story__card public-place-story__card--feature">
                ...
                <figcaption>
                  ...
                  <strong>Khu vực dân cư, đồi núi, du lịch và dịch vụ được quản lý trong cùng phạm vi hành chính.</strong>
                </figcaption>
              </figure>
              <figure className="public-place-story__card">
                ...
                <figcaption>
                  ...
                  <strong>Số liệu được quản lý theo từng thôn và kỳ báo cáo.</strong>
                </figcaption>
              </figure>
              <figure className="public-place-story__card">
                <picture>
                ...
                <figcaption>
                  ...
                  <strong>Chỉ dữ liệu đã được kiểm tra và phê duyệt mới hiển thị trên cổng.</strong>
                </figcaption>
              </figure>
            </div>
        ...
        <SectionCard className="mx-auto max-w-2xl p-5 md:p-8">
          ...
          <form onSubmit={lookupSubmission} className="mt-6 flex flex-col gap-3 sm:flex-row">
            <label className="flex-1 text-sm font-semibold text-slate-700">
              Mã tra cứu
              <input value={lookupCode} onChange={(event) => setLookupCode(event.target.value.toUpperCase())} maxLength={32} minLength={16} required className="mt-1.5 font-mono tracking-wider" placeholder="MÃ TRA CỨU" />
            </label>
          ...
          {lookupResult && (
            <div role="status" className="mt-5 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
              <div className="flex flex-wrap items-center gap-2">
                <span className="status-badge" data-tone="info">
                  <strong>Trạng thái:</strong> {getPublicStatusLabel(lookupResult.status)}
                </span>
                {lookupResult.case?.category ? (
                  <span className="status-badge">
                    <strong>Loại phản ánh:</strong> {getPublicCaseCategoryLabel(lookupResult.case.category)}
                  </span>
                ) : null}
              </div>
      ...
      {mode === "proposal" && (
        <SectionCard className="mx-auto max-w-3xl overflow-hidden">
          <div className="border-b border-slate-200 p-5 md:p-7">
            ...
            <ol className="mt-6 grid grid-cols-3 gap-2" aria-label="Tiến trình gửi đề nghị đối chiếu số liệu">
              {["Số liệu", "Liên hệ", "Xác nhận"].map((label, index) => {
                ...
                return (
                  <li key={label} aria-current={proposalStep === number ? "step" : undefined} className={`flex items-center gap-2 border-t-2 pt-3 text-xs font-semibold ${active ? "border-emerald-700 text-emerald-900" : "border-slate-200 text-slate-400"}`}>
                    <span className={`grid h-6 w-6 place-items-center rounded-full ${active ? "bg-emerald-800 text-white" : "bg-slate-100"}`}>{proposalStep > number ? <Check className="h-3.5 w-3.5" /> : number}</span>
                    {label}
                  </li>
                );
              })}
          ...
          <form onSubmit={handleSubmitProposal} className="p-5 md:p-7">
            {formError && (
              <div role="alert" className="mb-5 rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm font-semibold text-rose-800">
                {formError}
              </div>
            )}
            ...
            {proposalStep === 1 && (
              <div className="space-y-5">
                <div className="rounded-xl border border-sky-200 bg-sky-50 p-4 text-sm leading-relaxed text-sky-950">
                  ...
                  <p className="mt-1">Biểu mẫu áp dụng cho CT01, CT02, CT09, CT12 và CT13. Vấn đề về đường, điện, nước, rác thải hoặc an toàn được tiếp nhận tại mục phản ánh hiện trường.</p>
                  <Button type="button" variant="secondary" className="mt-3" onClick={() => setMode("case")}>
                    <MapPin />
                    Phản ánh hiện trường
                  </Button>
                </div>
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="text-sm font-semibold text-slate-700">
                    Thôn
                    <select className="mt-1.5" value={selectedVillageId} onChange={(event) => setSelectedVillageId(event.target.value)}>
                      {villages.map((item) => (
                        <option key={item.id} value={item.id}>
                          {item.name}
                        </option>
                  ...
                  <label className="text-sm font-semibold text-slate-700">
                    Kỳ công bố
                    <select className="mt-1.5" value={selectedPeriod} onChange={(event) => setSelectedPeriod(event.target.value)}>
                ...
                <div className="grid gap-4 lg:grid-cols-[minmax(0,1.35fr)_minmax(12rem,.8fr)_minmax(12rem,.8fr)]">
                  <label className="text-sm font-semibold text-slate-700">
                    ...
                    <select className="mt-1.5" value={selectedIndicator} onChange={(event) => setSelectedIndicator(event.target.value)}>
                  ...
                  <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3" aria-live="polite">
                    <p className="text-xs font-semibold text-slate-500">Giá trị đang công bố</p>
                    <p className="mt-1 text-lg font-bold text-slate-900">{hasSelectedPublishedValue ? `${formatPublicIndicatorValue(selectedPublishedValue)} ${selectedIndicatorMeta.unit}` : "Chưa có dữ liệu"}</p>
                    <p className="mt-1 text-xs text-slate-500">
                      {periodLabel} · {updatedLabel}
                    </p>
                  </div>
                  <label className="text-sm font-semibold text-slate-700">
                    Giá trị đề xuất <span className="font-normal text-slate-500">({selectedIndicatorMeta.unit})</span>
                    <input className="mt-1.5" type="number" min={0} required value={suggestedValue} onChange={(event) => setSuggestedValue(event.target.value)} />
                  </label>
                </div>
                <label className="block text-sm font-semibold text-slate-700">
                  Lý do cần đối chiếu
                  <textarea className="mt-1.5" rows={4} required value={explanation} onChange={(event) => setExplanation(event.target.value)} placeholder="Nêu nguồn thông tin và nội dung cần kiểm tra…" />
                </label>
            ...
            {proposalStep === 2 && (
              <div className="space-y-6">
                <section aria-labelledby="public-proposal-contact-heading" className="rounded-xl border border-slate-200 bg-slate-25 p-4 sm:p-5">
                  <h3 id="public-proposal-contact-heading" className="text-sm font-bold text-slate-900">
                    Thông tin người gửi
                  </h3>
                ...
                <div className="flex justify-between gap-3">
                  <Button type="button" variant="secondary" onClick={() => goToProposalStep(1)}>
                    <ArrowLeft />
                    Quay lại
                  </Button>
            ...
            {proposalStep === 3 && (
              <div className="space-y-5">
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                  <h3 className="font-bold text-slate-900">Kiểm tra trước khi gửi</h3>
                  <dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <dd className="font-semibold text-slate-900">{hasSelectedPublishedValue ? `${formatPublicIndicatorValue(selectedPublishedValue)} ${selectedIndicatorMeta.unit}` : "Chưa có dữ liệu"}</dd>
                    </div>
                    <div>
                      <dt className="text-slate-500">Giá trị đề xuất</dt>
                      <dd className="font-semibold text-slate-900">
                        {Number(suggestedValue).toLocaleString("vi-VN")} {selectedIndicatorMeta.unit}
                      </dd>
                    </div>
                    <div className="sm:col-span-2">
                      <dt className="text-slate-500">Lý do</dt>
                      <dd className="font-semibold text-slate-900">{explanation}</dd>
                    </div>
                  </dl>
                </div>
                <label className="flex min-h-14 items-start gap-3 rounded-lg border border-slate-200 p-4 text-sm text-slate-700">
                  <input type="checkbox" className="mt-0.5 h-5! min-h-5! w-5!" checked={privacyConsent} onChange={(event) => setPrivacyConsent(event.target.checked)} required />
                  <span>Tôi đồng ý gửi số điện thoại, thông tin tùy chọn đã nhập và nội dung đề nghị để UBND xã Bà Nà xử lý theo thông báo quyền riêng tư.</span>
                </label>
                <div className="flex justify-between gap-3">
                  <Button type="button" variant="secondary" onClick={() => goToProposalStep(2)}>
                    <ArrowLeft />
                    Quay lại
                  </Button>
                  <Button type="submit" disabled={isSending || !privacyConsent}>
                    {isSending ? <RefreshCw className="animate-spin" /> : <ClipboardCheck />}
            ...
            {proposalStep === 4 && (
              <div className="py-5 text-center">
                ...
                <h3 className="mt-5 text-xl font-bold text-slate-900">Đề nghị đối chiếu đã được ghi nhận</h3>
                <p className="mx-auto mt-2 max-w-lg text-sm text-slate-600">Lưu mã bên dưới để theo dõi trạng thái. Trang xác nhận không hiển thị lại thông tin cá nhân của bạn.</p>
                {trackingCode ? (
                  <div className="mx-auto mt-5 max-w-sm rounded-lg border border-emerald-200 bg-emerald-50 p-4">
                    <p className="text-xs font-semibold text-emerald-800">MÃ TRA CỨU</p>
                    <p className="mt-1 select-all font-mono text-xl font-bold tracking-wider text-emerald-950">{trackingCode}</p>
                    <Button type="button" variant="secondary" className="mt-3" onClick={() => void copyTrackingCode()}>
                      <Copy />
                      {trackingCodeCopied ? "Đã sao chép" : "Sao chép mã"}
                    </Button>
                  </div>
                ) : (
                  <EmptyState title="Chưa nhận được mã tra cứu" description="Vui lòng liên hệ bộ phận tiếp nhận nếu bạn cần kiểm tra trạng thái." />
                )}
