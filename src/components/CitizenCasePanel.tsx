...
export default function CitizenCasePanel({ villages, onBack }: { villages: Village[]; onBack: () => void }) {
  ...
  const captureLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => setError("Không lấy được vị trí. Hãy cấp quyền hoặc ghim vị trí trên bản đồ trong lần triển khai tiếp theo."),
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const data = await apiJson<CaseResult>("/api/cases", { method: "POST", body: JSON.stringify({
        ...
        submitter_phone: phone || null, consent_version: "2026-07-18", location_confirmed: Boolean(location),
