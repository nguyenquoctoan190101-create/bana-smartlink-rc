...
export default function PilotObservatory({ enabled }: { enabled: boolean }) {
  ...
  return <div className="grid gap-5 lg:grid-cols-2">
    <SectionCard>
      <div className="flex items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 text-lg font-bold"><Activity className="h-5 w-5 text-emerald-700" />Quan trắc gần nhất</h2><p className="mt-1 text-sm text-slate-600">Chỉ dùng để rà soát pilot nội bộ; chưa tự phát cảnh báo.</p></div><Button variant="secondary" onClick={() => void refresh()} disabled={loading}><RefreshCw className={loading ? "animate-spin" : ""} /></Button></div>
